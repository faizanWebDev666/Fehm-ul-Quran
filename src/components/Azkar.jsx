...
export const Azkar = () => {
  ...
  return (
    <section className="mx-auto max-w-7xl py-5 sm:py-8 lg:py-10 animate-fade-in" dir={isUrdu ? 'rtl' : 'ltr'}>
      ...
      <Sidebar compact />
      <div className="mt-5 grid gap-6 lg:mt-6 lg:grid-cols-[minmax(0,1fr)_19rem] lg:items-start"><main className="min-w-0"><div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#C9A66B]/20 bg-[#C9A66B]/[0.07] px-4 py-3 dark:bg-[#C9A66B]/[0.06]"><div className="flex items-center gap-2 text-sm font-bold text-[#1B4332] dark:text-[#C9A66B]"><CheckCircle2 className="h-4 w-4" />{completedCount} / {azkar.length} {t.completedAzkar}</div><div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400"><Clock3 className="h-4 w-4" />{repetitionCount} {ui.repetitions}</div></div><div className="space-y-4">{azkar.map((zikr, index) => { const current = counts[zikr.id] || 0; const complete = current >= zikr.count; return <article id={zikr.id} key={zikr.id} className={`scroll-mt-24 arch-card border bg-white p-5 shadow-sm transition-all sm:p-7 dark:bg-[#161D17] ${complete ? 'border-[#1B4332]/50 dark:border-[#C9A66B]/50' : 'border-[#C9A66B]/25'}`}><div className="mb-5 flex items-start justify-between gap-3"><div className="flex items-center gap-3"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#1B4332]/10 text-sm font-bold text-[#1B4332] dark:bg-[#C9A66B]/15 dark:text-[#C9A66B]">{index + 1}</span><div><h2 className="text-base font-bold text-[#1B4332] dark:text-[#EDEAE0]">{isUrdu ? zikr.titleUrdu : zikr.title}</h2><p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{zikr.count}× {isUrdu ? 'تکرار' : 'recitation'}</p></div></div><a href={zikr.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex max-w-[48%] items-center gap-1.5 rounded-full bg-[#C9A66B]/10 px-2.5 py-1 text-xs text-[#8E4E2B] transition-colors hover:bg-[#C9A66B]/20 dark:text-[#E7C992]" title={zikr.reference}><BookOpen className="h-3 w-3 shrink-0" /><span className="truncate">{zikr.reference}</span><ExternalLink className="h-3 w-3 shrink-0" /></a></div><p className="whitespace-pre-line font-quran text-right text-2xl leading-[2.25] text-[#1B4332] sm:text-3xl lg:text-[2.1rem] dark:text-[#EDEAE0]" dir="rtl">{zikr.arabic}</p><div className="my-5 h-px bg-[#C9A66B]/20" /><p className="text-sm italic leading-6 text-[#B0693F] dark:text-[#C9A66B]" dir="ltr">{zikr.transliteration}</p><p className="mt-3 text-base leading-8 text-gray-700 sm:text-lg dark:text-gray-300">{isUrdu ? zikr.urdu : zikr.english}</p><button onClick={() => increment(zikr)} disabled={complete} className={`mt-6 flex w-full items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-bold transition-all focus:outline-none focus:ring-2 focus:ring-[#C9A66B] focus:ring-offset-2 disabled:cursor-default ${complete ? 'bg-[#1B4332]/10 text-[#1B4332] dark:bg-[#C9A66B]/15 dark:text-[#C9A66B]' : 'bg-[#1B4332] text-[#FAF7F0] hover:bg-[#0D3B33] active:scale-[0.99]'}`}>{complete ? <CheckCircle2 className="h-5 w-5" /> : <span>{t.tapToCount}</span>}<span className="rounded-full bg-black/10 px-2 py-0.5 text-xs">{current} / {zikr.count}</span></button></article>; })}</div></main><div className="lg:sticky lg:top-5"><Sidebar /></div></div>
    </section>
  );
};
